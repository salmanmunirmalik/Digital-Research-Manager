...
-- Note: This migration requires unified_communications table to exist

-- Skip if table doesn't exist
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'unified_communications') THEN
        RAISE NOTICE 'unified_communications table does not exist, skipping seed data';
        RETURN;
    END IF;
    ...
    -- Add communication threads if table exists
    ...
    -- Add interactions if table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'communication_interactions') THEN
